"""
AHU two-stage v3 weighted delta model.

Builds on v2 (subsystem-specific sequence windows) and adds sample weighting
so that time steps with large true delta receive more influence during training.

Weight for each sample: w = 1 + weight_k * |delta_true|
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train AHU v3 weighted delta models.")
    ...
    parser.add_argument("--window", type=int, default=12, help="Past steps before t. 12 => [t-12 ... t].")
    parser.add_argument("--weight-k", type=float, default=5.0, help="Weight = 1 + k * |delta|.")
...
def build_dataset(rows: list[dict[str, str]], freq_min: int, window: int) -> SequenceDataset:
    ...
    for idx in range(window, len(rows) - 1):
        # skip any window that crosses a gap
        if any(
...
def compute_weights(deltas: list[float], k: float) -> list[float]:
    """w_i = 1 + k * |delta_i|.  Larger changes get proportionally more weight."""
    return [1.0 + k * abs(d) for d in deltas]
...
def train_weighted_ridge(
    ...
    n = len(features[0]) + 1  # +1 for intercept
...
def train_weighted_tree_ensemble(
    ...
    for _ in range(n_trees):
        # weighted bootstrap: sample with probability proportional to weight
        ...
        for _ in range(sample_size):
            ...
            while lo < hi:
                ...
                if cum[mid] < r:
                    ...
                    hi = mid
            indexes.append(lo)
        trees.append(build_tree(features, deltas, sample_weights, indexes, 0, max_depth, min_leaf, max_features, rng))
...
def fmt(split: str, model: str, v: dict[str, float]) -> str:
    return (
...
def closer_stats(
    ...
    """Returns (frac_closer_to_current, frac_closer_to_previous)."""
    closer_cur = closer_prev = equal = 0
    for t, p, prev in zip(true_vals, pred_vals, prev_vals):
        ...
        if d_cur < d_prev:
            ...
            equal += 1
    n = len(true_vals)
    return closer_cur / n, closer_prev / n
...
def write_predictions(
    ...
    for cfg in SUBSYSTEMS.values():
        t = cfg["target"]
        columns.extend([f"{t}_true", f"{t}_persistence", f"{t}_w_ridge", f"{t}_w_tree"])

    out_rows: list[dict[str, str]] = []
    for idx, ts in enumerate(test.timestamps):
        row: dict[str, str] = {"timestamp": ts}
        for subsystem, cfg in SUBSYSTEMS.items():
            target = cfg["target"]
            current = test.current_values[target][idx]
            feats = test.features[subsystem][idx]
            ...
            row[f"{target}_w_ridge"] = f"{predict_ridge(ridge, current, feats):.8g}"  # type: ignore[arg-type]
            row[f"{target}_w_tree"] = f"{predict_tree(tree, current, feats):.8g}"  # type: ignore[arg-type]
        out_rows.append(row)
...
def render_html(path: Path, rows: list[dict[str, str]], window: int, weight_k: float) -> str:
    series_keys = ["persistence", "w_ridge", "w_tree"]
    colors = {"persistence": "#9ca3af", "w_ridge": "#2563eb", "w_tree": "#dc2626"}
    labels = {"persistence": "Persistence", "w_ridge": "Weighted Ridge", "w_tree": "Weighted Tree"}
    ...
    plot_data = {
        ...
        "targets": {
            target: {
                "values": {
                    s: [float(r[f"{target}_{s}"]) for r in rows]
...
<head>
  ...
  <title>AHU v3 Weighted Delta 5-min Predictions</title>
  <style>
...
<body>
  <main>
    <h1>AHU v3 Weighted Delta — 5-min Predictions</h1>
    <p class="meta">Source: {html.escape(str(path))}<br />
    Time range: {first} to {last}. Rows: {len(rows)}.<br />
    Window: [t-{window} … t]. Sample weight = 1 + {weight_k} × |delta|.</p>
    <div class="note">
      V3 adds delta-proportional sample weighting to v2: samples where the true temperature
      change is large receive weight = 1 + {weight_k} × |delta|, forcing the model to learn
      larger changes rather than defaulting to persistence.
    </div>
    <div class="legend">
...
def main() -> None:
    ...
    for subsystem, cfg in SUBSYSTEMS.items():
        ...
        train_weights = compute_weights(train_deltas, args.weight_k)
        train_feats = train.features[subsystem]

        print(f"Training {subsystem} (target={target})...")
        ...
        report.append(f"Train samples: {len(train_feats)}  (weighted)")
        ...
        for split_name, ds in datasets.items():
            ...
            m_persist = metrics(true_vals, persist_pred)
            m_ridge = metrics(true_vals, ridge_pred)
            m_tree = metrics(true_vals, tree_pred)

            report.append(fmt(split_name, "persistence", m_persist))
            report.append(fmt(split_name, "w_ridge", m_ridge))
            report.append(fmt(split_name, "w_tree", m_tree))

            if split_name == "test":
                cc_r, cp_r = closer_stats(true_vals, ridge_pred, current_vals)
                cc_t, cp_t = closer_stats(true_vals, tree_pred, current_vals)
                report.append(f"  w_ridge  closer_to_X(t+1)={cc_r:.1%}  closer_to_X(t)={cp_r:.1%}")
                report.append(f"  w_tree   closer_to_X(t+1)={cc_t:.1%}  closer_to_X(t)={cp_t:.1%}")

        report.append("")

    # write outputs
    pred_path = out_dir / "ahu_v3_weighted_test_predictions_5min.csv"
    html_path = out_dir / "ahu_v3_weighted_predictions_5min.html"
    report_path = out_dir / "ahu_v3_weighted_model_report_5min.txt"

    predictions = write_predictions(pred_path, test, models)
    report_path.write_text("\n".join(report), encoding="utf-8")
    html_path.write_text(render_html(pred_path, predictions, args.window, args.weight_k), encoding="utf-8")
